# ===== v54 = full stack + QUADRATIC drift profile (toe -2.0, projection-matched) =====
...
sub[['id', 'tvt']].to_csv('submission.csv', index=False)
print('submission.csv rewritten (v54 = full stack + QUADRATIC drift profile (toe -2.0, projection-matched))')
...
import numpy as np, pandas as pd
DELTA = -2.0          # projection-matched: optimal quadratic toe under BOTH
                      # hypotheses (truth-quadratic: -1.5/0.75 = -2.0;
                      # truth-linear: best quad coeff = -1.875)
...
for w, grp in sub.groupby('well'):
    ...
    sub.loc[grp.index, 'tvt'] = tvt + DELTA * pos**2   # quadratic profile
    n3 += 1
print('stage 3: QUADRATIC drift ramp DELTA=%+.2f (toe-matched) on %d wells' % (DELTA, n3))
assert sub['tvt'].notna().all()
sub[['id', 'tvt']].to_csv('submission.csv', index=False)
