# ===== v56 = full stack + QUARTIC drift profile (toe -2.98; p-axis: 1->8.730, 2->8.722, 3->8.720) =====
...
sub[['id', 'tvt']].to_csv('submission.csv', index=False)
print('submission.csv rewritten (v56 = full stack + QUARTIC drift profile (toe -2.98; p-axis: 1->8.730, 2->8.722, 3->8.720))')
...
import numpy as np, pandas as pd
DELTA = -2.98         # p=4 dose from the four-point linear parabola (the
                      # best-validated object on the axis): -1.49 * 6/3 = -2.98.
                      # The chained route via p3 gives -2.74; evidence-weighted
                      # choice favors the parabola route.
...
for w, grp in sub.groupby('well'):
    ...
    sub.loc[grp.index, 'tvt'] = tvt + DELTA * pos**4   # quartic profile
    n3 += 1
print('stage 3: QUARTIC drift ramp DELTA=%+.2f (projection-matched) on %d wells' % (DELTA, n3))
assert sub['tvt'].notna().all()
sub[['id', 'tvt']].to_csv('submission.csv', index=False)
