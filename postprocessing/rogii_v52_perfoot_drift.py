# ===== v52 = v40 stack + length-PROPORTIONAL drift (per-foot rate, dose-matched to -1.5@median) =====
...
sub[['id', 'tvt']].to_csv('submission.csv', index=False)
print('submission.csv rewritten (v52 = v40 stack + length-PROPORTIONAL drift (per-foot rate, dose-matched to -1.5@median))')
...
import numpy as np, pandas as pd
RATE = -1.5 / 4800.0   # ft per station: dose-matched to the measured optimum
                        # (delta* = -1.5 at the median ~4800-station blind zone)
...
for w, grp in sub.groupby('well'):
    ...
    if len(idx) < 50 or not np.isfinite(tvt).all():
        continue
    dist = (idx - idx.min()).astype(float)          # stations from blind start
    sub.loc[grp.index, 'tvt'] = tvt + RATE * dist   # length-PROPORTIONAL drift
    n3 += 1
    toe_doses.append(RATE * dist.max())
print('stage 3: per-foot drift RATE=%.2e/station on %d wells | toe dose median %+.2f'
      ' ft, range [%+.2f, %+.2f]' % (RATE, n3, np.median(toe_doses),
                                     min(toe_doses), max(toe_doses)))
assert sub['tvt'].notna().all()
sub[['id', 'tvt']].to_csv('submission.csv', index=False)
